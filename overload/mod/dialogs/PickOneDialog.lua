...
function _M:init(title, optionlist, action)
	...
	--		value - the value to be passed to the action function when the item is selected
	self.optionlist = optionlist
	self.action = action
	self:generateList()
	print("[SkooBot] [PickOneDialog] Custom Action Dialog init with title "..title)
...
function _M:use(item)
	...
	print("[SkooBot] [PickOneDialog] Pick One Dialog option selected: "..item.value)
